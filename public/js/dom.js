...
var dquery = function (selector, context) {
  ...
  if (selector) {
    ...
    // Node/element
      ...
      selector.nodeType ||
      toString.call(selector) === toString.call(window) ||
      toString.call(selector) === toString.call(document)
    ) {
      arr.push(selector);
    }
      ...
      if (el.dataset) {
        ...
        for (var i = 0; i < el.attributes.length; i++) {
          ...
          if (attr.name.indexOf('data-') >= 0) {
            dataset[StringUtil.toCamelCase(attr.name.split('data-')[1])] =
              attr.value;
          }
        }
      }
    ...
    function handleLiveEvent(e) {
      ...
      if (dquery(target).is(targetSelector)) listener.call(target, e);
        ...
        for (var k = 0; k < parents.length; k++) {
          if (dquery(parents[k]).is(targetSelector))
    ...
    if (this.length > 0) {
      if (selector) {
        if (
          ...
          dquery(this[0].nextElementSibling).is(selector)
        )
    ...
    if (this.length > 0) {
      if (selector) {
        if (
          ...
          dquery(this[0].previousElementSibling).is(selector)
        )
  ...
  parent: function (selector) {
    ...
    for (var i = 0; i < this.length; i++) {
      if (this[i].parentNode !== null) {
        if (selector) {
          if (dquery(this[i].parentNode).is(selector))
      ...
      for (var j = 0; j < childNodes.length; j++) {
        if (!selector) {
          ...
          if (
            ...
            dquery(childNodes[j]).is(selector)
          )
            children.add(childNodes[j]);
        }
      }
    }
...
(function () {
  var shortcuts =
    'click blur focus focusin focusout keyup keydown keypress submit change mousedown mousemove mouseup mouseenter mouseleave mouseout mouseover touchstart touchend touchmove resize scroll contextmenu'.split(
      ' ',
    );
