...
function getCurvePath(
  ...
  const path =
    'M' +
    getNumberOfPx(start.style.left) +
    ' ' +
    getNumberOfPx(start.style.top) +
    ' Q ' +
    getNumberOfPx(control.style.left) +
    ' ' +
    getNumberOfPx(control.style.top) +
    ' ' +
    getNumberOfPx(end.style.left) +
    ' ' +
    getNumberOfPx(end.style.top);
  return path;
}
export default function Cannon() {
  ...
  const updatePaths = useCallback(() => {
    if (
      ...
      !end.current ||
      !control.current ||
      !quadraticCurve.current
    )
      return;
    const tan =
      (control.current.offsetTop - start.current.offsetTop) /
      (control.current.offsetLeft - start.current.offsetLeft);
    ...
    quadraticCurve.current.setAttribute(
      'd',
      getCurvePath(start.current, end.current, control.current),
    );
  }, []);
  ...
  useLayoutEffect(() => {
    ...
    end.current.style.left =
      rect.width - end.current.getBoundingClientRect().width + 'px';
    end.current.style.top = '236px';
    control.current.style.left =
  ...
  useEffect(() => {
    ...
    if (control.current) {
      makeDragalbe(
        control.current,
        function (el, pageX, startX, pageY, startY) {
          updatePaths();
        },
      );
    }
  ...
  return (
    <>
      <div
        ref={parentRef}
        className={`cannon-game relative hidden h-full w-full`}
      >
        <svg
        ...
        <CubicCircleButton
          className="absolute left-8 top-8 block h-[80px] w-[80px] text-white"
          onPointerDown={fire}
        >
          Fire
        </CubicCircleButton>
        ...
        <div
          ref={ballRef}
          className="absolute left-0 top-0 hidden h-10 w-10 animate-followpath rounded-full bg-red-500"
        ></div>
        <a
          ref={start}
          style={{
        ...
        >
          <CannonIcon className="h-[120px] w-[120px] fill-foreground stroke-orange-600 " />
        </a>
        <a ref={end} className="absolute block cursor-move">
          <BearIcon className="h-[120px] w-[120px] fill-foreground stroke-orange-600 " />
        </a>
        <a ref={control} className="absolute block cursor-move">
          <CalibrationIcon className="h-[60px] w-[60px] " />
        </a>
      </div>
      <div className="not-support-tip hidden p-10">
        Your browser does not support the path offset feature, please use a
        modern browser.
      </div>
      <style jsx>{`
